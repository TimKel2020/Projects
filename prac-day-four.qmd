...
The standard deviation is NA when all values in a group are NA or if there's only one data point in the group. 'OO' only has one value in the data set.

The standard deviation is 0 when all the values in the group are identical. Which is the case for 'YV', 'F9', 'AS' and 'HA'. They have one distinct value but multiple values as seen in above table.
...
# Q6
## i - identify routes that more than one airline flies
```{r}
...
## ii - for each such route, calculate the average arrival delay for each airline (exclude NAs). Find the names of these airlines.
```{r}
avg_delay <- mult_airlines |>
  group_by(origin, dest, carrier, distance) |>
  summarise(avg_arrival_delay = mean(arr_delay, na.rm = TRUE)) |>
  ungroup()

#avg_delay

# Join with airlines data to get airline names
avg_delay_named <- avg_delay |>
  left_join(airlines, by = "carrier") |>
  select(origin, dest, carrier, name, avg_arrival_delay) |>
...
## iii - for each such route, identify the airline with the worst and best average arrival delay
```{r}
performers <- avg_delay_named |>
  summarise(
    best_avg_delay = name[which.min(avg_arrival_delay)],
    worst_avg_delay = name[which.max(avg_arrival_delay)]
  )

performers
```

## iv -  the route with the greatest difference between the best and worst performing airlines
```{}
...
## v
Some airlines might have better flight scheduling, turnaround times, and on-time performance than others.

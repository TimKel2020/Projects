...
3. For linear regression, parameter estimates can be found as follows: Beta = ... 
 Here, Y is the response variable, and X is the design matrix. The cars data (an R data set, also always available in R) contains two variables: speed and distance to stop. Fit a simple linear regression model to these data, i.e. find the Beta estimates, using the equation above, and matrix calcuations in R.
...
Steps to Lowess
1. Data
As usual, we have observed data pairs
(x1,y1),…,(xn,yn)
...
wj=(1−(|xj−xi|dmax)3)3
where wj is the weight for neighbour xj, dmax is the distance to the farthest neighbour among the k neighbours selected.

Note that this means that if |xj−wi|≥dmax, then wj=0.

4. Weighted Regression
Perform a weighted linear regression y=β0+β1x+e where the weighting means that we want to minimise
∑j∈neighbourswj(yj−(β0+β1xj)2)

The optimal β is given by
β^=(XTWX)−1XTWy where W is the diagonal matrix given by the diagonal values (w1,...,wk).

The smoothed value for xi is then the predicted value y^i=β^0+β^1xi
...
- Generate y as a noisy sine wave such that
yi=sin(xi10)+ei
where ei∼N(0,0.22)
